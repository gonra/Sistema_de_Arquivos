...
public class EmpregadoController {
    ...
    @PutMapping(value = "/empregado/update-logged-user")
    public ResponseEntity<EmpregadoResponseDTO> updateLoggedUser(@RequestBody Map<String, String> data) {
        Empregado emp;

        Optional<Empregado> empregado = empregadoRepository.findById(Long.valueOf(data.get("idEmpregado")));
        if (empregado.isPresent()) {
            emp = empregado.get();
            emp.setNome(data.get("nome"));
